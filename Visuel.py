...
class Screen:
  ...
  style={"<>":"Screen.style_rhombus(P,color)","(<>)":"Screen.style_rhombus_curs(P,color)","O":"fill_circle(P,Screen.p,color)","(O)":"Screen.style_circle_curs(P,color)","*":"draw_croix(P,4,45,color)","+":"draw_croix(P,Screen.p,0,color)",".":"set_pixel(P.x,P.y,color)"}
...
def scatter(X,Y,color=Screen.palette['PrimaryColor'],style=None):draw_points([Point(round(Screen.center.x+x),round(Screen.center.y-y)) for x,y in zip(X,Y)],color,False,style)
def plot(X,Y,color=Screen.palette['PrimaryColor']):set_lines(connect_points([Point(round(Screen.center.x+x),round(Screen.center.y-y)) for x,y in zip(X,Y)]),color)
...
def draw_vector(P,V,color=Screen.palette['PrimaryColor']):x,y=milieu(P,P+V);draw_string(str(V.name),round(x-6),round(y-6),Screen.palette["PrimaryText"],Screen.palette["SecondaryText"]);draw_arrows([(P,V)],color,fill=False)
def draw_droite(P1,P2,color=Screen.palette['PrimaryColor'],name=None):
  ...
  else: _a=(P2.y-P1.y)/(P2.x-P1.x);_b=-(_a*P1.x-P1.y);P0,P3=Point(round((222-_b)/_a),222) if abs(P2.x-P1.x)<abs(P2.y-P1.y) else Point(0,round(_a*0+_b)),Point(round((-1-_b)/_a),-1) if abs(P2.x-P1.x)<abs(P2.y-P1.y) else Point(320,round(_a*320+_b));set_lines([(P0,P3)],color)
...
def draw_polygone(n,rayon,coord,color=Screen.palette['PrimaryColor']):alpha=2*pi/n;set_lines(connect_points([Point(round(coord.x+rayon*cos(k*alpha)),round(coord.y+rayon*sin(k*alpha)),color) for k in range(1, n+1)],True),color)
